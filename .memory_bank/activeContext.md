...
**Дата последнего обновления**: 2025-12-29 (последние изменения: улучшение UI, обновление Controls, рефакторинг левой панели)
...
- [x] Создать диалог New Map с проверкой несохраненных изменений (2025-12-29) ✅ ЗАВЕРШЕНО
  ...
  - [x] Визуальная валидация и корректировка соединений через UI `/generate-registry`
    ...
    - [x] Красивый вывод результатов vision анализа с типами соединений и их количеством
    - [x] Фиксированное позиционирование камеры (автоматическая подстройка под модель, без zoom)
    ...
    - [x] Проверка и корректировка соединений для всех типов тайлов через UI
    - [x] Интеграция локальных vision моделей для визуального определения типов соединений (2025-12-28)
      - [x] API endpoint `/api/llm/analyze-connections-vision` для vision анализа
      - [x] Утилита `TileVisionRenderer` для рендеринга скриншотов тайла с 6 углов
      - [x] UI интерфейс на странице `/generate-registry` для запуска vision анализа
      - [x] Автоматическое применение результатов vision анализа к editedConnections
      - [x] Поддержка OpenAI-совместимого формата для vision моделей
...
1. **✅ ЗАВЕРШЕНО: Улучшение диалога UnsavedDataDialog** (2025-12-29):
