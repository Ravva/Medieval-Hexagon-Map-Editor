...
- **Добавлен fallback подход**: если статический импорт не работает, используется fetch к статическому файлу
- Скопирован JSON файл в `public/lib/llm/tile-registry.json` для доступа через fetch
...
- **Добавлен многоуровневый подход загрузки**: статический импорт → fetch → динамический импорт
- Скопирован JSON файл в public папку для доступа через HTTP

**Следующие шаги**:
1. Дождаться деплоя (commit: 118b753) - GitHub Actions должен обновить ветку gh-pages
2. Проверить логи в браузере - должно показать результат тройного fallback подхода
